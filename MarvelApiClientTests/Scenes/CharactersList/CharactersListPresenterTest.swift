...
class MockCharactersListInteractor: GetCharactersListInteractorInputPort{
  ...
  func execute() {
    DispatchQueue.global().async {
      self.presenterDelegate?.interactorCompletion!(self.mockCharactersListInteractorData!)
    }
  }
}
...
//    mockView = CharactersListPresenterToViewMock(presenter: sut)
    ...
    mockInterator.presenterDelegate = sut
    // Then
...
//    mockView = nil
  ...
  func testIsLoading() throws {
    ...
    sut.cellPresentationModels.valueChanged = { [weak self] (_) in
      DispatchQueue.main.async {
